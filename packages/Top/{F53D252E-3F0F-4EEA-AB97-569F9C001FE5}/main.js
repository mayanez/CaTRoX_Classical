...
var g_component_scrobble = _cc('foo_scrobble');
...
function TopPanel() {
    ...
    function create_buttons(ww, wh) {
        ...
        if (g_component_scrobble) {
            buttons.buttons.scrobble = new _button(x, y, w, h, is_scrobbling_enabled ? button_images.LastFM_Disable : button_images.LastFM_Enable, function () {
        ...
        buttons.buttons.search_yt = new _button(x, y, w, h, button_images.SearchYT, function () { fb.RunMainMenuCommand('View/Youtube Source/Search on Site'); }, 'Search Youtube');
        x += w + p;

        if (is_youtube) {
            buttons.buttons.yt_video = new _button(x, y, w, h, is_youtube_video_displayed ? button_images.VideoHide : button_images.VideoShow, function () {
    ...
    function create_button_images() {
        ...
        var accented_ico_colors =
            [
                _RGB(182, 158, 44), // _RGBA(255, 220, 55, 155) + g_theme.colors.pss_back
                _RGB(234, 202, 53), // _RGBA(255, 220, 55, 225) + g_theme.colors.pss_back
                _RGB(141, 122, 38)  // _RGBA(255, 220, 55, 105) + g_theme.colors.pss_back
            ];
